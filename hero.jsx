    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 28, alignItems: 'center', fontFamily: 'var(--cc-font-sans)', fontSize: 14.5, justifySelf: 'center' }}>
        {[
          ...
          }} onClick={(e) => { if (x.href === '#cart') { e.preventDefault(); window.openCart && window.openCart(); } }}>{x.label}</a>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 18, alignItems: 'center', color: '#3a2a2a', justifySelf: 'end' }}>
...
const Hero = ({ headline, sub, accent }) => (
  <section style={{ background: '#fffafa', borderBottom: '1px solid #f5d3d3' }}>
    <div style={{
      display: 'grid', gridTemplateColumns: '1fr 1.15fr', minHeight: 620,
    }}>
      {/* LEFT — copy */}
      <div style={{
        display: 'flex', flexDirection: 'column', justifyContent: 'center',
        padding: '80px 64px 80px 80px', position: 'relative', borderRight: '1px dashed #f5c0c0',
      }}>
        <div style={{
        ...
        <h1 style={{
          fontFamily: 'var(--cc-font-serif)', fontWeight: 500, fontSize: 68,
          ...
          {headline.split('\n').map((l, i) => (
            <div key={i}>
              {i === 1
                ? <span style={{ fontFamily: 'var(--cc-font-script)', fontWeight: 400, color: '#f08a8a', fontSize: 70 }}>{l}</span>
                : l}
            </div>
          ))}
        </h1>
        <p style={{
          fontFamily: 'var(--cc-font-sans)', fontSize: 16.5, lineHeight: 1.65,
